...
<html>
  ...
  <body>
    <!-- 타이틀 시작-->
    <div class="container">
      <h2 class="font-weight-bold mt-5" style="font-family: 'Nanum Myeongjo', serif;">공지사항</h2>
    </div>
    <br><br>
        ...
        <table class="table table-bordered table-hover">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="cursor_test"><c:out value="관리자" /></td>
    ...
    <div class="container">
      <div class="d-flex justify-content-end mb-5">
        <c:if test="${memberInfo.member_id == 'admin' }">
          <button
            type="button"
            class="btn btn-outline-secondary btn-sm mr-2" onclick="javascript:location.href='${contextPath}/admin/notice/writeView.do'">
            게시물 등록
          </button>
        </c:if>
      </div>
			...
			<nav aria-label="Page navigation">
				<ul class="pagination">
					<c:if test="${pageMaker.prev}">
					    <li class="page-item"><a class="page-link; btn btn-outline-secondary" href="${contextPath}/notice/list.do${pageMaker.makeSearch(pageMaker.startPage - 1)}" aria-disabled="true">이전</a></li>
					</c:if> 
					    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
					    	<li class="page-item" <c:out value="${pageMaker.cri.page == idx ? 'class=info' : ''}" />>
					    	<a class="page-link" href="${contextPath}/notice/list.do${pageMaker.makeSearch(idx)}">${idx}</a></li>
					    </c:forEach>
					
					    <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
					    	<li class="page-item"><a class="page-link" href="${contextPath}/notice/list.do${pageMaker.makeSearch(pageMaker.endPage + 1)}">다음</a></li>
					    </c:if>
				</ul>
			</nav>
		</div>
